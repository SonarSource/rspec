...
. click on the grey _Run wokflow_ button (on the right).
. in the field _"ID of an existing rule"_ write the ID of the existing rule you want to add language to. It must be in a form `S####` where `####` is the number of the rule. write the list of languages you want to specify this rule for.
. in the field _"Language to be added to the rule"_ specify one language to be added to the rule that it does not cover yet. +
