...
Unprocessed PRs are the ones without any label. You can easily see all of them with the filter [`Label > Unlabeled`](https://github.com/SonarSource/rspec/pulls?q=is%3Aopen+is%3Apr+no%3Alabel). +
All processed PRs should have at least one label that corresponds to the bubble(s) the PR is related to. This allows bubbles to easily filter out the PRs they are interested in.
