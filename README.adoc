...
Multi-language rule creation is more involved than the default process because it involves multiple roles that typically do not conicide.
...
`"extra"."coveredLanguages"` contains the languages rule is implemented for.

The workflow below makes sure that for all rules on the main branch are implemented for all languages they are specified for or for all languages listed in `"coveredLanguages"`.
...
.. Alice adds Ada to `"coveredLanguages"` (`"extra": {"coveredLanguages": ["Ada"]}`).
...
. Bob opens a new RSPEC PR "Create rule S100[cobol]" to add Cobol to `"coveredLangauges"` (`"extra": {"coveredLanguages": ["Ada", "Cobol"]}`).
