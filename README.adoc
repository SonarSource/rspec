...
** `rule.adoc`: write down the whole description of the rule. If you want to include already existing parts (that are in the parent directory for example), use the `include::` syntax.
...
You found a bug, something is bothering you or you have an idea to improve the project? First have a look at all the https://jira.sonarsource.com/projects/RULEAPI/issues/RULEAPI-324?filter=allopenissues[open tickets]. If you don't see anything related to your subject, please open a new ticket in the backlog, with `backlog` as fix version.
