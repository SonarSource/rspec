...
* `infeasible`: it is not feasible to propose a quickfix for any of the issues produced by the rule.
* `targeted`: it is possible to implement automatic quickfixes at least for some of the issues produced by the rule, but none are implemented right now.
