...
Raise on loops that take a variable as a condition and use `sleep` inside.
We might want to not raise if the sleep is not the last statement in the loop if there are FPs on Peach.


=== Message
Refactor the loop to use an `Event` instead of polling with `sleep`.
