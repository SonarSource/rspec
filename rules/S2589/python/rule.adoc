...
If, on close inspection, it becomes apparent that the condition is actually
unnecessary, it can be removed together with the associated control flow
construct (e.g. the `if`-statement containing the condition).
...
In the first example, the gratuitous condition `a` is a `True` constant.
