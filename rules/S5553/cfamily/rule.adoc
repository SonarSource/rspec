...
A reference-to-`const` directly bound to a temporary object extends lifetime of this object.
...
std::cout <<c <<std::endl; // c refers to a the memory formerly occupied by a temporary string
----
