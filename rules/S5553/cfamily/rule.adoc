...
Note that the interface using `std::string_view` is still mostly prone to the same problems,
however, it avoids the performance impact of `std::string` creation.
