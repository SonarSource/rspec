...
Alternatively, using `std::string_view` does not escape this flaw (see below)
but it avoids the performance impact of `std::string` creation
and can be safe in some situations.

Here is a possible implementation using `std::string_view`
(or its equivalent for code that cannot use {cpp}17):
