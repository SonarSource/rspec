...
When a workflow invokes an action with untrusted input parameters, and that action forwards them unchecked to a shell interpreter, the interpreter may treat those inputs as shell commands causing a command injection vulnerability.

Note, that actions implemented in other languages (such as JavaScript) are not directly vulnerable to this issue, as long as they do not evaluate the input as code.
...
If this untrusted data is given as a parameter to an invoked action, which directly interpolates it into shell commands using GitHub's expression syntax (`${{ ... }}`), it can lead to script injection vulnerabilities.
...
For instance, if a pull request title contains shell metacharacters like `$(whoami)` or `; rm -rf /`, these can be executed as commands when the called workflow executes, potentially compromising the build environment or exposing sensitive information.
...
To prevent potential command injection in called GitHub Actions workflows, avoid directly interpolating untrusted data passed into the invoked actions.
Instead, use environment variables to safely pass untrusted data without risk of code execution even if the receiving action passes them as-is into a step executed by a shell interpreter.
...
The following workflow is vulnerable to command injection if the `helloaction` passes the `message` parameter directly to a shell interpreter.
