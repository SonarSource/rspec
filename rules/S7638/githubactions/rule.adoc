...
To fix this issue, remove setting of the `ACTIONS_ALLOW_UNSECURE_COMMANDS` environment variable.
If your workflow relies on functionality provided by `::set-env` or `::add-path` commands, you can replace them with the recommended alternatives:

* To set environment variables, use the `GITHUB_ENV` file. For example, to set an environment variable named `VARIABLE` to `value`, you can use the following command in your workflow step:

  # This is equivalent to `export VARIABLE=value` across stpes
  echo "VARIABLE=value" >> $GITHUB_ENV

* To prepend a new entry to the system PATH, use the `GITHUB_PATH` file. For example, to add `/new/path` to the PATH, you can use the following command:
