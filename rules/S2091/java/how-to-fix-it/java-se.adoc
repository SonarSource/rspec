...
injected in an Xpath query without prior validation.
...
In the example, a sanitization mechanism is applied to an XPath Factory as a
resolver to resolve the untrusted data to the request in a safe way, similarly
to parametrized SQL queries.
