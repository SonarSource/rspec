...
`Assert.True` and `Assert.False` are not raising this issue for xUnit because `Assert.Fail` was introduced in 2020 version 2.4.2 and both `Assert.True(false, message)` and `Assert.False(true, message)` were ways to simulate it.
