...
A cast is an https://learn.microsoft.com/en-us/dotnet/visual-basic/programming-guide/language-features/data-types/implicit-and-explicit-conversions[explicit conversion], in other words, this is a way to tell the compiler the intent to convert from one type to another.
...
Dim i As Integer = CType(x, Integer)              // Casting (explicit conversion) from float to int
Dim y As Integer = DirectCast(x, Integer)        // Casting using DirectCast operator from float to int
----
...
Module Program
    Sub Main()
        Dim Orphan As New OrphanClass
        Dim x As IMyInterface = DirectCast(Orphan, IMyInterface)    ' Noncompliant: InvalidCastException is being thrown
    End Sub
End Module
----
