...
[source,python]
----
def setCoordinates(x1, y1, z1, x2, y2, z2): # Noncompliant
    # ...
----
...
def setSize(width, height, depth):
   # ...
----
...
This rule raises an issue when a function, a method, or a lambda has more parameters than the given number.
