...
In order to properly make our `TemplateWrapper` generic, we need to add the necessary `std::same_as` constraint in addition to `std::constructible_from`.
...
public:
  ...
  template<typename U>
    requires (!std::same_as<std::remove_cvref_t<U>, TemplateWrapper>) && std::constructible_from<T, U>
