...
==== Compliant solution
If you are using `actions/upload-artifact` version older than 4.4, upgrading it to a newer version will fix this by automatically ignoring hidden files from being uploaded.
...
If you want to use `persist-credentials: true` & `include-hidden-files: true`, explicitly include `!.git` so that `.git` directory is not uploaded:
