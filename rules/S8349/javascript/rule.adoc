...
* `media-src https://example.com/` blocks loading and execution of `<audio>` and `<video>` tags whose src is not exactly the value
* And so on. The full list of directives can be found in Mozilla's https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy#directives[Content-Security-Policy (CSP) header | MDN].
