...
Moreover, unlike `Trace.TraceError`, `Trace.WriteLineIf(switch.TraceError, ...)` would behave like `Trace.WriteLine(...)` when `switch.TraceError` is `true`:

* always writes to the underlying trace listeners directly
* bypasses all filters related to log levels
* and doesn't categorize the log entry with the `Error` level

More information about the potential issues of using `Trace.WriteLine` can be found in S6670. 
