...
Running long-running blocking IO operations on the default thread pool:
----
class workerClass {
...
== Compliant Solution
Running long-running blocking IO operations in an appropriate thread pool using `Dispatcher.IO`:
----
class workerClass(
