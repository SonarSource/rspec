...
{cpp}20 introduced conditional `explicit(expr)` that allows the developers to make a constructor or conversion conditionally explicit depending on the value of `expr`.
The new syntax allows an constructor or conversion operator declared with an `explicit(expr)` specifier, to be implicit in case when `expr` evaluates to `false`.
The issue is not raised in such situation.

Additionally, developers can use `explicit(false)` to mark the constructors or conversion operators as intentionally implicit.
