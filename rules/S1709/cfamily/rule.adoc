...
This makes it possible to declare implicit constructor or conversion operator with `explicit` specifiers for which condition evalutes to `false`.
The issue is not raised in such situation.

In addition this makes it possible to use ```++explicit(false)++`` to mark corresponding constructor or conversion operator as intentially implicit.
