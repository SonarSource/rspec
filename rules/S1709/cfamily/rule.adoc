...
{cpp}20 introduced conditional `explicit(expr)` that allows the developer to make a constructor conditionally explicit depending on the value of `expr`.
The new syntax enables the developer to declare an implicit constructor or conversion operator with an `explicit(expr)` specifier where `expr` evaluates to `false`.
The issue is not raised in such situation.

Additionaly, this enables the developer to use `explicit(false)` to mark the constructor or conversion operator as intentionally implicit.
