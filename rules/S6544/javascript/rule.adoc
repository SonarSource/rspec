...
If you mistook the promise with its resolved value, await or resolve the promise so it gets resolved correctly, using an IIFE if necessary:
...
If you forgot to handle the promise rejection, you should add it either with `.catch(error => {/*...*/})` or `.then(result => {}, error => {})`.
