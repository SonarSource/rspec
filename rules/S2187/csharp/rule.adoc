...
It does not apply to `xUnit` since it does not require a https://xunit.net/docs/comparisons#attributes[test class attribute].
...
Using a base class that contains one or more test cases, to provide generic test cases is also consider a compliant scenario.

==== Classes that contain `AssemblyInitialize` or `AssemblyCleanup` methods

*This particular exception scenario is only applies to the MSTest test framework.*
