...
`constexpr` functions can produce a compile-time constant in some contexts and when they are called with appropriate arguments, but they can also be invoked at run-time.
`consteval` functions must always be evaluated at compile-time and, if that cannot happen, a compilation error will occur.
