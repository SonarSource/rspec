...
deserializes untrusted data that can contain objects without validating it first.
...
soon as the `Loader` component `SafeLoader` is passed to it. All other loaders
are considered insecure, as they may allow deserialization of objects.
