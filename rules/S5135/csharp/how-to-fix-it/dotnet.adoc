...
{
    [HttpPost]
    public ActionResult Deserialize(HttpPostedFileBase inputFile, string expectedTypeName)
    {
        ExpectedType expectedObject = null;
        Type type                   = Type.GetType(expectedTypeName);
...
In the code samples, a pre-approved class is used natively by XmlSerializer to
validate the class during serialization. +
The name of this expected class should not be untrusted.
