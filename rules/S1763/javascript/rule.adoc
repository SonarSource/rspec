...
Unreachable code is the code whose statements cannot be executed under any circumstances. Jump (``++return++``, ``++break++`` and ``++continue++``) and ``++throw++`` statements move control flow out of the current code block. So any statements that come after a jump are effectively unreachable.

Unreachable statements can be a sign of a logic error in the code and can lead to unexpected behavior at runtime.
