...
If only Basic Authentication is available, enable it too.
...
resource "data_factory_linked_service_odata" "example" {
  basic_auth {
    username = local.creds.username
    password = local.creds.password
  }
...
Preliminary Note: App Service and equivalents resources:
...
** Highlight the resource if ``auth_settings`` is missing
** Highlight ``auth_settings->enabled = false`` regardless of ``auth_settings->unauthenticated_client_action = "AllowAnonymous"``
** Highlight ``auth_settings->unauthenticated_client_action = "AllowAnonymous"``
* For ``api_management_api``: Highlight the resource if ``openid_authentication`` is missing
* For ``api_management``:
