...
* For fixed-size arrays, where the memory is on the stack, use ``++std::array++``. It is like a C-style array, except that it has a normal argument passing semantics, and the size is always a part of the type. If ``++std::array++`` is not available to you (before {cpp}11), you can roll your version.
...
The rule S945 is related to this but focuses on passing arguments of an array type. S5025 will flag the dynamic memory allocation that could be replaced by ``++std::vector++``.
...
This rule will not report the use of C-style arrays in

* ``++extern "C"++`` code, since those arrays are often required here for compatibility with external code
* The arguments of ``++main++``
* The arguments of ``++constexpr++`` and ``++consteval++`` functions, and ``++constexpr++`` and ``++constinit++`` variables for the case of ``++char++``arrays (i.e. strings).



== See

* https://github.com/isocpp/CppCoreGuidelines/blob/46dadd0b16b904fe0ff88c1fef9b0702dfd139e2/CppCoreGuidelines.md#slcon1-prefer-using-stl-array-or-vector-instead-of-a-c-array[{cpp} Core Guidelines SL.con.1] - Prefer using an STL array or vector instead of a C array
