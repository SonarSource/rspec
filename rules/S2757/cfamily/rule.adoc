...
Using operator pairs (e.g., ``++=+++``) that look like reversed single operators (``+++=++``) is confusing. They compile and run but do not produce the same result as their mirrored counterpart.
