...
Consider the call ``a::b::f()``, and the following cases for the name specified to the list:

- If the function name is not qualified, the call will match by name (``f`` will cause a match, but ``g`` will not).
- If the function name is partially qualified, the call will match by name and namespaces (``a::b::f`` and ``b::f`` will cause a match, but ``a::f`` will not).
- If the function name is fully qualified, the call will be matched by qualified name lookup (only ``::a::b::f`` will cause a match with the call).
