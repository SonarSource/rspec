...
Consider the following declaration ``namespace a { namespace b { void f(); } }``, and the following cases for how this function could be added to specified to the list:

- If the function name is not qualified (list contains ``f``), any call to any function ``f`` will be trigger an issue, regardless of declaration scope.
- If the function name is partially qualified (list contains ``b::f``), any call to a function ``f`` declared inside a scope named ``b`` will trigger an issue
(calling the function ``namespace c { namespace b { void f(); } }`` will trigger the issue).
- If the function name is fully qualified (list contains ``::a::b::f``), only calls to the function ``f``, from the named scope ``b``, from the global named scope ``a`` will trigger an issue

It is recommended to provide fully qualified names to the configurable list. This also applies to functions in the global scope.
