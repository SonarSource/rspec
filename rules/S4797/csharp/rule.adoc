...
The creation of <code>FileStream</code> from a <code>SafeFileHandle</code> won't raise an issue as the creation of <code>SafeFileHandle</code> already raised one.
Only the creation of <code>StreamWriter</code> or <code>StreamReader</code> from a string path will raise an issue for the same reason.
