...
With the first and third options, the implementation is reusing existing functions and types by delegating the comparison operation to them.

When working with `std::string`, the last option may often be the simplest thanks for `std::string_view`.

For instance, to implement a transparent case-insensitive comparator for strings, we can leverage the fact that ``++string_view++`` offers lightweight conversions. Furthermore, this class provides the same functionality we need from `std::string`, which allows us to update `CaseInsensitiveCompare` by changing two lines:
