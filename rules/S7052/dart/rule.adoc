Variables and fields that are not initialized inline should be explicitly type, even when the type is correctly inferred at the first initialization, and the variable doesn't end up being `dynamic`.
