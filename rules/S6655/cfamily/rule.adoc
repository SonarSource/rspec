Accessing local objects outside of their scope (for example via pointer
taken inside the scope) has undefined behavior. This rule flags such access
...
In the above example, scopes `A` and `B` limit the operation that is performed in
