...
`Switch(true)` is a used construction when matching against expressions instead of values. Expressions in the `case` clauses will be evaluated before matching, meaning any expression that evaluates to `true` will be matched

[source,javascript]
----
function weekStatus (value) {
  let day;
  switch (true) {
    case (value === 'MON' || value === 'TUE' || value === 'WED' || value === 'THU' || value === 'FRI'):
      day = "Week";
      break;
    case (value === 'SAT' || value === 'SUN'):
      day = "Weekend";
      break;
  }
  return day;
