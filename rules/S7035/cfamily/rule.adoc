...
Or, since {cpp}11, users could use the more robust type_trait:
...
Unscoped enums with no underlying type specified have an implementation-defined implicit underlying type. Calling `std::to_underlying` on them would make the code less stable so this rule doesn't raise on them.
