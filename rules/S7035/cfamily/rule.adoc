...
The result of casting any integer type to `bool` does not depedent on the specific integer type.
For same reason casting enumeration to `bool` does not depend on the underlying type of enumerator,
and this rule does not raise issue for such cast.
