...
Instead, the best way to see whether a variable is equal to `NaN` is to use the https://learn.microsoft.com/en-us/dotnet/api/system.single.isnan[float.IsNaN] and https://learn.microsoft.com/en-us/dotnet/api/system.single.isnan[double.IsNaN] methods, which work as expected.
