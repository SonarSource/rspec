== How to fix it in .Net

=== Code examples

include::../common/fix/code-rationale.adoc[]

==== Noncompliant code example

include::../common/fix/rsa.adoc[]

[source,cpp,diff-id=1,diff-type=noncompliant]
----
using System;
using System.Security.Cryptography;

public void encrypt()
{
    var RsaCsp = new RSACryptoServiceProvider(); // Noncompliant
}
----

include::../common/fix/dsa.adoc[]

[source,cpp,diff-id=2,diff-type=noncompliant]
----
using System;
using System.Security.Cryptography;

public void encrypt()
{
    var DsaCsp = new DSACryptoServiceProvider(); // Noncompliant
}
----

include::../common/fix/ec.adoc[]

[source,cpp,diff-id=3,diff-type=noncompliant]
----
using System;
using System.Security.Cryptography;

public void encrypt()
{
    ECDsa ecdsa = ECDsa.Create(ECCurve.NamedCurves.brainpoolP160t1); // Noncompliant
}
----

==== Compliant solution

[source,cpp,diff-id=1,diff-type=compliant]
----
using System;
using System.Security.Cryptography;

public void encrypt()
{
    var RsaCsp = new RSACryptoServiceProvider(2048);
}
----

[source,cpp,diff-id=2,diff-type=compliant]
----
using System;
using System.Security.Cryptography;

public void encrypt()
{
    var Dsa = new DSACng(2048);
}
----

[source,cpp,diff-id=3,diff-type=noncompliant]
----
using System;
using System.Security.Cryptography;

public void encrypt()
{
    ECDsa ecdsa = ECDsa.Create(ECCurve.NamedCurves.nistP256);
}
----

=== How does this work?

include::../../common/fix/fix.adoc[]

=== Pitfalls

==== The KeySize Property of CryptoServiceProviders

Attempts at changing the KeySize property of CryptoServiceProviders will fail
to update the property without raising an error. This should not be considered
a workarond.
To change the key size, use one of the constructor overloaded with the targeted
key size.


=== Going the extra mile

include::../../common/extra-mile/pre-quantum.adoc[]
