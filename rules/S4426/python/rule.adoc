...
* For ``++EC{cpp}`` (elliptic curve cryptography) algorithms key size should be at least 224 bits long
* For ``++RSA++`` public key exponent should be at least 65537.

This rule raises an issue when an ``++RSA++``, ``++DSA++`` or ``++EC{cpp}`` key-pair generator is initialized using weak parameters. 
