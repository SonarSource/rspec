...
This rule raises an issue when the ``++EIBRESP++`` field is compared directly to a variable or hard-coded numeric value that is not wrapped in ``++DFHRESP++`` function.
