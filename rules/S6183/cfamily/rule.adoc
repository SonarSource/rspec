...
This rule reports comparisons between signed and unsigned integers, only when the signed argument cannot be proven to be negative. Otherwise, S6214 will trigger instead. As a consequence, if this rule is enabled, S6214 should be enabled too.
...
[source,cpp]
----
bool less = 2U < -1; // Compliant, but will raise S6214
...
* S6214 - a version of this rule that only triggers when it detect negative values are involved.
