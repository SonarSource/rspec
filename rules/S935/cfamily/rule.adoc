...
Furthermore, the issue is not raised for the coroutines, introduced in {{cpp20}}, that always return a coroutine object but may not do it explicitly from its body (when `return_void` function is defined in the promise type of the coroutine).
