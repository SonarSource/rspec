...
Furthermore, the issue is not raised for the coroutines (introduced in {{cpp20}}) that always return the coroutine object, but may
not expect value to be returned from its body (when `return_void` function is defined in promise type used by the coroutine).
