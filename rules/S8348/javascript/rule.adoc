...
a `Server B` if its origin is not the exact same tuple.
...
Enabling third-party manipulation of the `Access-Control-Allow-Origin` header
exposes the application to a varity of different attacks, namely XSS or TLS
hijacking. +
The situation is worsened if the `Access-Control-Allow-Credentials` is set to
`true` or injectable.

== How to fix it

Generally, do not use third party data to set up CORS boundaries. Instead,
create a strict allow-list of origins.
