...
Additionally, using `inplace=True` may trigger a `SettingWithCopyWarning` and make the overall intention of the code unclear. In the following example, modifying `df2` will not modify the original `df1` dataframe, and a warning will be raised:
