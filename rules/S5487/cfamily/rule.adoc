...
About this life cycle, the following patterns should be avoided as they result in undefined behavior:
...
void double_initialization(void)
{
  ...
  pthread_mutex_init(&mtx1, NULL); // Noncompliant
}
...
void destroy_locked(void)
{
  ...
  pthread_mutex_destroy(&mtx1); // Noncompliant
}
...
void lock_destroyed(void)
{
  pthread_mutex_init(&mtx1, NULL);
  pthread_mutex_destroy(&mtx1);
  pthread_mutex_lock(&mtx1); // Noncompliant
}
...
void ok2()
{
  Mutex m(&mtx1);
  LockGuard lock(m);
  // mutex will unlock automatically at the end of the scope
}
----
