...
C++ provides facilities to lock multiple _mutexes_ in one go, with a dedicated deadlock prevention algorithm. They should be used instead. Before C++17, you should use ``std::lock``, and since C++17 you can use a variadic constructor of ``std::scoped_lock``. See the examples for more details.
