...
GCP also offers to give public access to authenticated users. The risk is the
same regarding data theft. The only thing that changes is that it enables to
track user access in case of an incident.
...
Explicitly set the access to this resource or feature to be private.
...
* For containers clusters: 
** Omission: Omitting {attribute} grants public access to parts of this cluster.
** Explicitly set to false: Make sure that granting public access is safe here.
* For the rest: Make sure that granting public access to this resource is safe here.
* For ACL resources: Make sure that granting public access to this resource is safe here.
* For DNS managed zones omissions: Omitting {attribute} grants public access to this managed zone. Make sure it is safe here.

=== Highlighting

* Assignments: Highlight the sensitive assignement
* Lists: Highlight the sensitive item
