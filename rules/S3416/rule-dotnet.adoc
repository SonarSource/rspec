...
The rule doesn't raise issues when custom handling of logging names is in place. For example, when:
- the non-generic version of `ILoggerFactory.CreateLogger`, taking a string as its input parameter, is used
- and the name of the logger is not derived from a `Type`
