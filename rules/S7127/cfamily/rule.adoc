...
When all uses of the `ARRAY_SIZE` macro are removed, the macro should also be removed. 
However doing so is not required to address issues raised by this rule,
