...
Class Example
    ...
    Public Sub DoSomethingWithMonitor()
        SyncLock obj ' Compliant: the lock will be release at the end of the SyncLock block
            If IsInitialized() Then 
                ' ...
            End If
        End SyncLock
...
Class Example
    ...
    Public Sub DoSomethingWithReaderWriteLockSlim()
        lockObj.EnterReadLock() ' Compliant: the lock will be release in the finally block
