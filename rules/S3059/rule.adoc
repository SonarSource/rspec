There's no point in having a ``++publi{cpp}`` member in a non-``++publi{cpp}`` class because objects that can't access the class will never have the chance to access the member.
