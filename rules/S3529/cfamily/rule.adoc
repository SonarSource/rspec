...
Depending on the type, passing an _RAII_ object operations may either allocate a new block of memory and copy the elements (`std::vector`),
transfer ownership of the memory to constructed object (`std::unique_ptr`), or shared ownership and free memory when the last object is destroyed (`std::shared_ptr`).
