...
To fix this issue, make sure the expression used in an `expect` statement is an exception which derives from `BaseException`/`Exception` or a tuple of such exceptions.
...
try:
    ...
except [TypeError, ValueError]:  # Noncompliant: lists or sets are not accepted.
    print("exception")
----
