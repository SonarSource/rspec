...
When a method in a derived class has the same name as a method in the base class but with a signature that only differs by types that are less derived (for example `object` vs `string`), the result is that the base method becomes hidden.
