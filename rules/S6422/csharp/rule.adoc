Making blocking calls to ``++async++`` methods transforms something that was intended to be asynchronous into a synchronous block. Doing so inside an Azure Function can lead to thread exhaustion.
