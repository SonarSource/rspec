Non-abstract classes and enums with non-``++stati{cpp}``, ``++private++`` members should explicitly initialize those members, either in a constructor or with a default value.
