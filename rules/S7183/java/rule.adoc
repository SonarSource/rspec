...
The framework implies that the method signatures of methods annotated with `@InitBinder` have a `void` return type.
When this is not the case, the controller contaning the annotated method will throw a runtime exception when invoked.
