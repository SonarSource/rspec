...
This compliant solution adds a guardrail to the triager agent to ensure that
user instructions are properly vetted before being passed to secondary agents,
and then that the triager only passes along instructions that are appropriate.
...
guardrail_agent = Agent(
    ...
    instructions="Check if the user is asking about the Example Topicf.",
