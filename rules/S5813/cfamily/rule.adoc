...
The function `size_t strlen(const char *s)` measures the length of the string `s` (excluding the final null character). The `wcslen` does the same for wide characters, and should be used with the same guidelines.
