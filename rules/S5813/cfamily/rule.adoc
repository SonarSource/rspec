...
`strlen` and `wcslen` process strings under the assumption that they are not null pointers.
...
When a string is missing the null character at the end, these functions may continue to iterate in memory and the behavior of the program is undefined.
...
* Use safer functions. The C11 functions `strlen_s` and `wcslen_s` from annex K handle typical programming errors.
  Note, however, that they have a runtime overhead and require more code for error handling and therefore may not be suited to every case.
