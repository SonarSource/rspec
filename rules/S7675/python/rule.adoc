This is an issue when copying existing PyTorch tensors using deprecated methods like `torch.tensor()` or `tensor.new_tensor()` instead of the recommended `tensor.clone().detach()` or `tensor.detach().clone()`.
