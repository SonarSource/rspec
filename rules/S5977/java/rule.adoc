...
In almost all cases, it leads to problems and should be avoided.

Fundamentally, we aim to make tests deterministic and reproducible, primarily for the following reasons:

* When a test fails, the ability to reproduce the exact conditions that led to the failure is crucial for effective debugging.
...
This rule excludes the use of library functions that read the system clock (`++new Date()++`, `++Instant.now()++`, and so on).
...
By initializing a pseudo-random number generator with a fixed seed, tests can generate sequences of seemingly random data that are entirely reproducible across different test runs.
