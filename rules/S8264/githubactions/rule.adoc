...
The following workflow grants `contents: read` permission at the workflow level, which applies to all jobs:
...
jobs:
  ...
  checkout-main:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4 # Does not need contents: read permission
        with:
          ref: main
----
