...
The options for addressing order of initializations issues for non-`const` global variables are limited,
especially in case when older {cpp} standard.
The most robust solution of refactoring the global variable into static function variable,
requires replacing references to variable with the call to newly used funcitons.

For some projects and code bases, this may be too expensive, or even infesible.
In such case, you may consider replacing global variable with reference variable that initialized with result of getter function.
Such variable should be local (e.g. declared `static`) to avoid multiple defintions errors:
...
// source2.cpp
#include "header.h"
std::string s2 = s1 + " in other file"; // Compliant: `s1` is reference to `var`, that is initialized as part of `getS1()` call
----
