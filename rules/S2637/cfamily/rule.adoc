...
Ensure not to pass null values when non-null arguments are expected.
This especially holds for library functions such as the C standard library, for instance, which frequently requires `nonnull` pointer parameters.
