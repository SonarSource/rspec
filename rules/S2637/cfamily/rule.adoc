...
Passing null (i.e., `NULL`, `0` or `nullptr`) argument to a parameter as `nonnull`
...
This attribute specifies only the function call contract
...
struct List {
  int value;
  List* next; // NULL for a tail node.
};
...
size_t len(List *n) {
  if (!n) return 0; // Impossible branch according to the attribute
  return 1 + len(n->next);
}
----
...
Ensure not to pass null values when `nonnull` arguments are expected and not to return a null value when a function marked as `returns_nonnull`.
