...
The `String` type offers an indexer property that allows you to treat it as an array of chars, therefore, if you just need to access a specific character or iterate over all of them, the `ToCharArray` call should be omitted. Not omitting, for these particular cases, makes the code less performant as `ToCharArray`, copies the characters from the string into a new Unicode character array and also harder to read.

Same principle applies to https://devblogs.microsoft.com/dotnet/csharp-11-preview-updates/#utf-8-string-literals[utf-8 literals types] (`ReadOnlySpan<byte>`, `Span<byte>`).
