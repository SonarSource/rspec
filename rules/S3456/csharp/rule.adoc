...
The `string` type offers an indexer property that allows you to treat it as a `char array, therefore, if you just need to access a specific character or iterate over all of them, the `ToCharArray` call should be omitted. Not omitting, for these particular cases, makes the code harder to read and also less performant as `ToCharArray`, copies the characters from the `string object into a new Unicode character array.
