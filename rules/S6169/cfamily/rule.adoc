...
=== Why issue is raised for non-compile time variables?

For some variables, the compile will try to initialize them at compile time,
and only if such initialization is not possible, will evaluted them at runtime.

This happesns in two scenarios:

* For variable with static and thread storage duration, like gobal, static, and thread local variables.
+
[source,cpp]
----
int x = 10;           // Evaluted at compile time
int const y = init(); // Evaluted at compile time, if `init()` is constant-expression
std::mutex m;         // Evaluted at compile time, because default constructor is constexpr
...
* For local variables that are declared `const` and have integral and enumeration type.
+
[source,cpp]
----
void cpp03Code() {
...
----
+
Evaluating such variables at compile time was introduced in first {cpp} standard before `constexpr` was intorduced, to allow patterns like above.
It is recommended to mark such variables as `constexpr` since {cpp}11.

Due the above special rules, the `std::is_constant_evaluated()` and `if consteval` will always be `true` within such implicit constant initialization.
This may lead to surprising and intuitive result, thus this rule raises issues in such cases:
...
An immediate (including one marked `consteval`) function can only be invoked at compile-time,
...
[source,cpp]
----
consteval int handle(int);

constexpr void foo(int x) {
  return handle(x); // ill-formed, the process cannot be called at compile-time,
                    // because `x` may be only know at runtime
}
----
...
constexpr void foo(int x) {
  if consteval {
    return handle(x); // OK, handle is evaluted only at compile-time
  }
}
----
