...
* For variable with static storage duration, like gobal and static variables.
+
[source,cpp]
----
...
* For local variables that are declared `cosnt` and have integral and enumeration type.+
+
[source,cpp]
----
void cpp03Code() {
...
----
+
Evaluting such variables at compile time was introduced in first {cpp} standard before `constexpr` was intorduced, to allow patterns like above.
...
This may lead to suprising and intuitive result, thus this rule raises issues in such cases:

[source,cpp]
----
void onlyRuntime() {
  bool const ce = std::is_constant_evaluted(); // Noncomplaint: always true, due implicit constant evaluation
  bool ce = std::is_constant_evaluted();       // Noncomplaint: always false not implicit constant evaluation
}

constexpr bool possiblyCompileTimeFunc() {
  bool const ce = std::is_constant_evaluted(); // Noncomplaint: always true, due implicit constant evaluation
  bool ce = std::is_constant_evaluted();       // Complaint: depends on the call site
}
----
...
An immediate (`consteval`) function can only be invoked at compile-time,
