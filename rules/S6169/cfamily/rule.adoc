...
* All expressions inside an _immediate context_. This includes bodies of `consteval` function, `then` branches of `if consteval`, and `else` branches of `if not consteval`.
+
[source,cpp]
----
