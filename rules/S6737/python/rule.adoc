...
Even thought the ``++DataFrame.groupby++`` method call instantiates the ``++GroupBy++`` object it only verifies that a valid mapping is passed - no real data splitting occurs until it’s needed.
