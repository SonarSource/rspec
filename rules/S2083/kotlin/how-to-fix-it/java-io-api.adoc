...
class ExampleController {
    companion object {
        private val targetDirectory: String = "/path/to/target/directory/"
    }
...
class ExampleController {
    companion object {
        private const val targetDirectory = "/path/to/target/directory/"
        private val targetPath = File(targetDirectory).toPath().normalize()
    }

    @GetMapping("/delete")
    @Throws(IOException::class)
    fun delete(@RequestParam("filename") filename: String) {
        val file = File(targetPath.toString() + filename)
