...
public void endpoint(@RequestParam("folder") folderName) throws IOException {

    String canonicalizedFolderName = folderName.getCanonicalPath();
...
terminating path separator in their return value and cannot be used for
validation. +
Validation code should be tested to ensure that it cannot be thwarted by this
issue.
