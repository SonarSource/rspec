...
It's not always clear whether a variable is guaranteed to be non-null or not so it's easy to misuse this operator. In Kotlin if you pass null where null is not expected, you will get a NullPointerException. However, if you have a Java interop then you can have unexpected behavior at runtime. 
