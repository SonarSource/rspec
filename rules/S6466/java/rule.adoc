...
This bug can cause your program to crash or behave unexpectedly, and it can also lead to security vulnerabilities.
...
In this first example, we create an array `arr` containing three elements.
...
* https://docs.oracle.com/javase/8/docs/api/java/util/Arrays.html#stream-T:A-[Javadoc for `Arrays.stream()`]
