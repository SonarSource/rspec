When a cycle exists between classes during their ``++stati{cpp}`` initialization, the results can be unpredictable because they depend on which class was initialized first.
