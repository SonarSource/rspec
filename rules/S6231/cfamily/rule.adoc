...
void caller(std::string const& s) {
  ...
  std::vector<int const*> nums = getNums();
  if (oddAre0(std::span<int const*>{nums.begin(), nums.end()})) { // Noncompliant: This copy is verbose and slow
    // ...
  }
}
----
