Assigning the address of an automatic object to another object that outlives it is dangerous since the address is invalidated after the first object has ceased to exist.

== Why is this an issue?

If the address of an automatic object is assigned to another automatic object of larger scope, a static object, or if it is returned from a function (using `return` or an output parameter), then the object containing the address may exist beyond the time at which the original object ceases to exist.
...
void bar() {
  ...
  str_ptr = str; // Noncompliant: address of `str` will become invalid when `str` goes out of scope
}
----
...
void caller() {
  int array[256];
  buz(array, sizeof(array) / sizeof(array[0]));
}
----
