...
This rule raises an issue when specific records from the <code>Trigger</code> are referenced, i.e. when it finds one of the following patterns:

* <code>Trigger.new[x]</code>
* <code>Trigger.old[x]</code>
* <code>Trigger.oldmap.get\(x\)</code>
* <code>Trigger.newmap.get\(x\)</code>

where <code>x</code> is a hardcoded number.
