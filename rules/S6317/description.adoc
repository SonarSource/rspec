...
The example above is just one of many permission combinations that the rule can detect. There are other varieties based on ``++iam:PassRole++``, ``++lambda:UpdateFunctionCode++``, and other IAM permissions that allow to update, create, and attach IAM policies.
