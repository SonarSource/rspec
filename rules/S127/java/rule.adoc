...
A `for` loop termination condition should test the loop counter against an invariant value which does not change during the execution of the loop.
...
* When the stop condition depends on a method call
...
Make the termination condition invariant by using a constant or a local variable instead of an expression that could possibly change during execution of the loop.
