...
This rule works for private properties `#x` as well as public ones `x`.

The `getX()` and `setX()` and `get x()` and `set x()` syntaxes are supported for methods.

The rule is enforced if the property with the corresponding name exists, like `x` for `getX()`.
