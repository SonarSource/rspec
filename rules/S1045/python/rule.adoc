...
When using `except` statement, make sure to:
* Catch first the most specialized exceptions, i.e when wanting to catch a `FloatingPointError`, as `FloatingPointError` is a subclass of `ArithmeticError`, the first `except` statement should be `FloatingPointError`.
* Catch the same exception only once.
* Catch `BaseException` with either an `except: BaseException` statement or a bare `except:` statement, as the two statements are equivalent.
