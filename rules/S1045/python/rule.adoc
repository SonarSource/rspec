...
* Order the `expect` blocks with the most specialized exceptions first, i.e when wanting to catch a `FloatingPointError`, as `FloatingPointError` is a subclass of `ArithmeticError`, the first `except` statement should be `FloatingPointError`.
