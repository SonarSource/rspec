...
* In Python 3, `BaseException` is the parent of every exception class. When a `BaseException` is caught by a `except` clause, none of the subsequent `except` statement will catch anything. This true as well for the bare except statement (`except:`).
...
** To catch a `BaseException`, make sure to use either an `except BaseException:` statement or a bare `except:` statement, as the two statements are equivalent.
...
*Note*: __It is generally not recommended to try catching `BaseException`, as it encompasses exceptions asking the interpreter to stop, such as `KeyboardInterrupt` and https://docs.python.org/3/library/exceptions.html#GeneratorExit[`GeneratorExit`] which is not an error. See https://www.python.org/dev/peps/pep-0352/#exception-hierarchy-changes[PEP 352] for more information. In order to catch all other exception you may want to use `Exception` instead.__
