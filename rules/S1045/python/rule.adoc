...
* In Python 3, `BaseException` is the parent of every exception class. When a `BaseException` is caught by a `except` clause, none of the subsequent `except` statement will catch anything. This is true as well for the bare except statement (`except:`).
