...
The rule is targeting test methods that lack an assertion and consist solely of an action and optionally a setup. 
Such tests are only verifying that no exception is thrown, without providing any guarantees regarding the behavior of the code under test.
Those tests increase the coverage without enforcing anything on convered code, resulting in a false sense of security.
...
In addition to using built-in assertion methods, you also have the option to create custom assertion methods. To do this, declare an attribute named `[AssertionMethodAttribute]` and apply it to the respective method. This allows you to encapsulate specific validation logic within your custom assertion methods.
Here's an example:
...
{
    [TestMethod]
    public void Add_SingleNumber_ReturnsSameNumber() => Validator.Caclulator(0); // Compliant
}

public static class Validator
{
    [AssertionMethod]
    public static void Caclulator(int value) =>
		new StringCalculator().Add(value.ToString()) == value;
}
