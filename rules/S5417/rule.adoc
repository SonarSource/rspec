...
* ``std::forward`` has a single use case: to cast a templated function parameter of type _forwarding reference_ (``T&&``) to the value category (``lvalue`` or ``rvalue``) the caller used to pass it. This allows ``rvalue`` arguments to be passed on as ``rvalues``, and ``lvalues`` to be passed on as ``lvalues``. This scheme is known as _perfect forwarding_.
Note that the standard states that "_a forwarding reference is an rvalue reference to a cv-unqualified template parameter that does NOT represent a template parameter of a class template_". Refer to the last non compliant code example.
