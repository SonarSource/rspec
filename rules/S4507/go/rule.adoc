...
Loggers should be configured so that debug-level information is not logged (debug-level is called `Info` in the case of GORM). The `DB.Debug` method should not be used as it creates a new `Session` with a logging level set to `Info`.
