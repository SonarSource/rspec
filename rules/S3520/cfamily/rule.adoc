...
While better, this is not a perfect solution.
Any raw pointers of references to memory held by _RAII_ objects may still lead to a "double-free".
...
The following example will demonstrate two `std::shared_ptr` objects being created, with an intention to point to the same object.
...
Due to the previously mentioned condition, when `p2` is destroyed, it will attempt to destroy the string as well, leading to a "double-free".
...
Similar to `std::unique_ptr`, a "double-free" can lead from an erroneous initialization.
This also comes from a confusion raised by the constructor overloads of this class.
...
void doSomething(char const* data) {
  ...
  // ...
  std::unique_ptr<std::string> p4(p3.get()); /// Noncompliant: Intention was to transfer ownership, double-free will happen.
...
This will not accept a pointer to object as a parameter.
...
Extra care must be taken when passing the address of the memory contained in a RAII object.
INCOMPLETE: TL; DR - Do not use std::shared/unique_ptr::get to send ptr out of scope. Do not release and delete manually because of deletors.
