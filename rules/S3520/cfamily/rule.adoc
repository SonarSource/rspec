...
Similar to `std::unique_ptr`, a "double-free" can occur from an erroneous initialization.
...
Passing this pointer to a call that could release the memory could cause a "double-free", amongst other problems.
