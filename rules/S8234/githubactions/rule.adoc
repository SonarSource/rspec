...
If a workflow with read-all permission is compromised, attacker can to sensitive data in issues, pull requests, or packages etc.
...
By specifying individual permissions like permissions: contents: read, GitHub restricts the token to only that scope. If a step tries to access other resources (e.g., reading issues with only contents: read), GitHub returns 403 Forbidden, preventing unauthorized acces.
