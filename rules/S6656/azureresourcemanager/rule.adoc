...
If `properties.expressionEvaluationOptions.scope` or `properties.expressionEvaluationOptions` is not set at all, then highlight `properties` entirely (of the `"Microsoft.Resources/deployments"` resource.)
