...
``++IDisposable/IAsyncDisposable++`` variables returned from a method or passed to other methods are ignored, as are local ``++IDisposable/IAsyncDisposable++``s that are initialized with other ``++IDisposable/IAsyncDisposable++``s.
