...
Another similar situation is when references are involed: the compiler deduces a value instead without an explicit return type.
...
Here are two such examples where the explicit return type introduces useful implicit conversions.
