...
int foo() {
  ...
  int z = 300; // Noncompliant: dead store
  z = 400;
  return x + z * 2;
}
----
...
When removing unused values and superfluous code, make sure that the right-hand side of a given assignment has no side effects that update the program's state.
...
Still, values that are never read such as `n_items` indicate code smells that should be mitigated.
...
int foo(int i) {
  ...
  size_t n_items = fwrite(str, sizeof(char), strlen(str), stdout); // Noncompliant: `n_items` is never read, but the call has side effects
  return i + 9001;
}
----
...
A function that is marked `nodiscard` or that returns an enumeration or class declared `nodiscard` by value which is used in a discarded-value expression encourages the compiler to issue a warning.
