...
- HttpClient, by default, doesn't respect the Time To Live (TTL) settings of the DNS. This means that if the IP address associated with a domain name changes, HttpClient might still use the old, cached IP address, leading to failed requests.
