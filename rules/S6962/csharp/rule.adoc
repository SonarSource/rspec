...
In case you decide to cache the HttpClient in a singleton or a static field, you should be aware that by default the HttpClient doesn't respect the Time To Live (TTL) settings of the DNS. This means that if the IP address associated with a domain name changes, HttpClient might still use the old, cached IP address, leading to failed requests.
...
public class FooController : Controller
{
    ...
    public async Task<string> Foo()
    {
        using var client = _clientFactory.CreateClient(); // Compliant
        return await client.GetStringAsync(_url);
    }
}
----
