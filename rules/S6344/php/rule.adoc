...
PHP doesn't fail if two or more `define` statements assign a value to the same constant name. In such a case,
PHP only issues a warning and ignores the second and further `define`.
...
In the worst case, the additional `define` statements attempt to assign a different value and
are followed by code which wrongly assumes the value of the constant. This can lead to difficult bugs.
