...
This rule raises an issue on Azure Functions when there is no logging statement inside a ``++catch++`` block, or if the logging is done at ``++Debug++`` or ``++Trace++`` level.
