...
However, this may not be obvious to everyone when reading the code and can cause needless confusion.
...
The problem is even more challenging to identify when virtual functions are indirectly called.
...
1. If subclasses can exist and customize the program's behavior, there is a bug. +
...
If you want to perform virtual calls during object construction that consider the actual type of the object, you should defer those calls after the object constructor has finished with a factory function.
...
You can deal with this issue with https://seanmiddleditch.github.io/enabling-make-unique-with-private-constructors/[the passkey idiom].
...
Or you can allow subclasses as long as they do not override `addChild` by marking only this function as `final`.
...
// We could have a "Going the extra mile" section about how to do without inheritance. It would however require collecting a good set of alternative designs, like composition over inheritance, using the decorator pattern, ... None of these solutions are universal so we cannot blindly suggest things.
