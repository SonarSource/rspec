This rule raises an issue when a special methods raises `NotImplementedError` instead of returning `NotImplemented`.
...
The ``++__length_hint__++`` special method also requires to return a `NotImplemented`. Its behaviour differs from the other methods, because when this special method returns `NotImplemented` the caller will return a default value. See https://peps.python.org/pep-0424/[PEP 424] for more information.
