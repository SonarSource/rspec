While rare, memory leaks can still occur in Java, and ``++stati{cpp}`` collections are a classic source. This is because ``++stati{cpp}`` class variables hang around for the life of a program. Unless the contents are diligently managed, items added to the collection shortly after start up will still be there in the collection - and in memory - until shutdown. Thus, you should be very certain that ``++stati{cpp}`` collections are both required and well-managed. 

This rule raises an issue on each ``++stati{cpp}`` collection.
