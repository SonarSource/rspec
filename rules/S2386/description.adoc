There is no good reason to have a mutable object as the ``++publi{cpp}`` (by default), ``++stati{cpp}`` member of an ``++interface++``. Such variables should be moved into classes and their visibility lowered. 
