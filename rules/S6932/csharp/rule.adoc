...
`Request.Form`, `Request.Headers`, `Request.Query` and `Request.RouteValues` are key/value collections that expose data from the incoming HTPP request:
...
|`Request.Form["id"]`
|optional `[FromForm]` attribute on the parameter
