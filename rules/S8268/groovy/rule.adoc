This rule raises an issue when `Thread.sleep()` or `sleep()` is called within a loop construct (while or for loop) where the thread is waiting for a condition to change.
...
Modern Java provides better alternatives through the `java.util.concurrent` package, which offers efficient, event-driven synchronization mechanisms that don't waste resources.
...
Replace busy waiting with `CountDownLatch.await()` for one-time events. This blocks the thread efficiently until the condition is met.
 ...
 * Java Concurrency Utilities - https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/package-summary.html[Official documentation for java.util.concurrent package with synchronization utilities]

 * CountDownLatch Documentation - https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/CountDownLatch.html[Documentation for CountDownLatch synchronization aid]

 * BlockingQueue Documentation - https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/BlockingQueue.html[Documentation for BlockingQueue interface for producer-consumer scenarios]
 ...
 * RSPEC-2142 - https://rules.sonarsource.com/java/RSPEC-2142/["wait(...)" should be used instead of "Thread.sleep(...)" when a lock is held]
