...
There is a common pattern is Javascript to overwrite certain parameters depending on other optional arguments being passed. For example, a callback is by convention always passed in the last position, meaning that if a parameter is a previous position was not passed, the callback will be passed in its position instead.

Because of this pattern, parameter reassignments which are inside an `if` statement block will not raise an issue.
