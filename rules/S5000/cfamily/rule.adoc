The function ``++memcmp++`` only returns meaningful results for objects of trivially copyable types that have no padding. This includes scalar types, arrays, and trivially copyable classes.

== Why is this an issue?

`memcmp` compares the raw memory representation of two objects (what the standard calls their _object representation_). For objects that are not trivially copyable or that contain padding, two objects storing identical values could have different raw memory representations, so the result of `memcmp` is not meaningful.
