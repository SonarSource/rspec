...
When working with a generic type of exception, the only way to distinguish multiple exceptions is to either check their message or their code, which is error-prone and difficult to maintain.
...
At best, this makes the code hard to understand and maintain. At worst, legitimate exceptions may be unintentionally silenced and errors may be hidden.
