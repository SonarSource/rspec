...
Unreachable is by definition not executed, thus `{null-keyword}` values will never be accessed.

include::{exceptionCodeUnreachable}[]

==== Validated value by analysis attributes [[null-analysis-attribute]]

https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/attributes/nullable-analysis[Nullable analysis attributes] provides more accurate semantic information when performing static analysis when it comes to detecting variable null-state. Thus, potential `{null-keyword}` values validated by one of the following attributes will not raise:
...
When values are validated to not be `{null-keyword}` with https://learn.microsoft.com/en-us/dotnet/api/system.diagnostics.debug.assert[Debug.Assert] it is safe to access it.

include::{exceptionCodeDebugAssert}[]
