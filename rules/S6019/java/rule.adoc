...
----
"start123endstart456end".replaceAll("start\\w*?(end)?", "x"); // Noncompliant. In contrast to what one would expect, the result is not "xx"
...
----
"start123endstart456end".replaceAll("start\\w*?(end|$)", "x"); // Result is "xx"
str.matches("\d*");
----
