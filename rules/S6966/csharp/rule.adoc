...
Using a synchronous method instead of its asynchronous counterpart in an `{keyword_async}` method blocks the execution and is considered bad practice for several reasons:
...
Instead of blocking, it's recommended to use the await keyword with async methods. This allows the system to release the current thread back to the thread pool until the awaited task is complete, improving scalability and responsiveness.
...
{
    stream.Read(arr, 0, 1024);             // Noncompliant
    stream.Dispose();                      // Noncompliant
    semaphoreSlim.Wait(cancellationToken); // Noncompliant
    qry.ToList();                          // Noncompliant in Entity Framework Core queries
    qry.FirstOrDefault(x => x.Age > 18);   // Noncompliant in Entity Framework Core queries
}
----
...
public async Task Examples()
{
    await stream.ReadAsync(arr, 0, 1024);
    await stream.DisposeAsync();
    await semaphoreSlim.WaitAsync(cancellationToken);
    await qry.ToListAsync();
    await qry.FirstOrDefaultAsync(x => x.Age > 18);
}
----
