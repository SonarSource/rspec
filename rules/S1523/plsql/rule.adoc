...
Any code which is dynamically evaluated in your process will have the same permissions as the rest of your code. Thus it is very dangerous to do so with code coming from an untrusted source. https://www.owasp.org/index.php/Code_Injection[Injected Code] can either run on the server or in the client (exemple: XSS attack).
...
The best solution is to not run code provided by an untrusted source. If you really need to build a command string using external parameters, you should use <code>EXECUTE IMMEDIATE</code> with bind variables instead.
