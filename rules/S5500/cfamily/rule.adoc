...
was introduced in {cpp}20 timeframe, and retrocativelly applied to previous standard. 
As a consequence, depending on the compiler version behavior of such return statement
is not consistient accross complies.
Furthermore, with the {cpp}20 rules, implicit move is not triggred if the function
returns reference:
[source,cpp]
----
Shape&& updateShape(Shape&& shape) {
...
For all of above, we do not tree `return p` as exception in {cpp} standard prior to {cpp}23,
...
is not applicable to function parameters, so such explicit move have no impact on 
optimizations.
