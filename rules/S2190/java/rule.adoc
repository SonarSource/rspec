...
Recursion can be a useful tool unless a recursive method does not correctly
detect base cases that let it break out of the recursion and ``++return++``.
...
2. The recursive method must contain a *termination condition* that checks
...
topic, we refer to external articles on the topic, see the section
*Articles & blog posts* under the tab *More Info*.
