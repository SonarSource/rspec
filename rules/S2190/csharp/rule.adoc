...
the https://en.wikipedia.org/wiki/Call_stack[call stack] will reach its limit and the program will crash due to the stack overflow exception.
...
In this example, `Pow` will keep calling `Pow` with `exponent - 1` forever, until the program crashes with a https://learn.microsoft.com/en-us/dotnet/api/system.stackoverflowexception[StackOverflowException].
