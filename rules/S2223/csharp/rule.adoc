...
Unlike instance fields, that can only be accessed by code having a hold on the instance, `static` fields can be accessed by any code having visibility of the field and its type.
...
Another typical scenario of use of non-private mutable `static` field is the following:
