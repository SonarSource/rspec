...
2. Locking on a `readonly` non-`private` field allows external code to be able to lock on the same field, potentially interfering with the synchronization.  For example a method from `Class A` can use a public object that is used for locking in another method in `Class B` and as a result we can have two unrelated methods waiting for each other.
...
4. Locking on a field of another type can interfere with the synchronization of methods of that type, and should be avoided.  
