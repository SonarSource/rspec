...
Cipher.getInstance("AES/CBC/PKCS5Padding"); // Noncompliant: Vulnerable to padding Orable attacks

Cipher.getInstance("RSA/None/NoPadding"); // Noncompliant: RSA without OAEP padding scheme is not recommanded
----
