In PHP, some constructs require parenthesis and some does not.
When they are not required, we should avoid using them.

== Why is this an issue?

Using constructs with parenthensis can be misleading at it will produce a syntax which look like a normal function call.
However those constructs having a lower precedence in the evaluation order than some others operators, this can lead to a behavior completely different from what we would expect at first look.
...
== How to fix it
When not necessary, do not use the parentheses for the constructs.
