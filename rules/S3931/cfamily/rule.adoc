...
In C++, it is common to null-check a pointer by using it as a condition. In such cases, if the pointer is ``NULL``, the condition evaluates to ``false``, and otherwise it is ``true``. Such pointer assignments are ignored: 
