...
Note: This scenario can only happen before {cpp}17. From {cpp}17, the standard states that each argument needs to be thoroughly evaluated before evaluating the other arguments. So the explicit construction of `unique_ptr` and `shared_ptr` become exception-safe.

*Performance*

Constructing `shared_ptr()` explicitly requires two heap allocations: one for the managed object and the other for the control block that stores data about the ref-counts and the `shared_ptr()` deleter. `make_shared()` still impacts performances, but it performs only one heap allocation. +
`make_unique()`, on the other hand, doesn't impact performances.
