...
* If the replacement string contains references like ``++$<Name>++``, they will also be treated as literals, but only if there are no named captures in the regular expressions, otherwise they will be replaced with the empty string.
