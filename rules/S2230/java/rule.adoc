...
Marking a non-public method `@Async` or `@Transactional` is both useless and misleading because Spring does not recognize non-public methods, and so makes no provision for their proper invocation. Nor does Spring make provision for the methods invoked by the method it called.
