...
// inclusion:
// Protect well-meaning users from the actions of malicious users by ensuring that user-supplied data
// cannot break out of its context when injected into a response. 
// To do this, encode the data according to its context. The following table lists how user input
// should be handled depending on its output context.

==== In the View, Model, or ViewModel 
:official-doc: https://docs.microsoft.com/en-us/aspnet/core/security/cross-site-scripting?view=aspnetcore-6.0
...
* Razer does not HTML-encode it at runtime to avoid double-encoding.
* Any data that is not programmatically HTML encoded before it is sent to Razor is unsafe.
