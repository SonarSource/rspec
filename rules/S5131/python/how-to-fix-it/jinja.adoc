...
[source,python,diff-id=32,diff-type=noncompliant]
----
...
def hello(request: Request, name: str):
    return templates.TemplateResponse('hello.html', {'request': request, 'hello': hello})
----
...
[source,python,diff-id=32,diff-type=compliant]
----
from fastapi import Request
from fastapi.templating import Jinja2Templates
