...
Injecting user-contoled value inside a ``++script++`` is dangerous and the best practice, in such a case, is to add the value to an attribute.
Another option is to use ``++tojson++`` filter to insert a data structure in the JavaScript code at render time.
