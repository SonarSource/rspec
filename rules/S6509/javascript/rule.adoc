In JavaScript, every value has an associated boolean value: either ``true`` or ``false``.

Values that are associated with ``true`` are said to be _truthy_, and those associated with ``false`` are said to be _falsy_.
