...
* Classes need to indicate that they are interested in serialization support by either
  . Applying a `[Serializable]` attribute
  . Declaring `ISerializable` in their base type list
  . Having an https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/serialization#supporting-runtime-serialization[serialization constructor]
