...
Character literals starting with ``\`` and followed by one or more digits are octal escaped literals. Same reasoning applies to such literals.
...
// or
char myChar = '\n'; // Use the common notation if it exist for the literal
----
...
* ``'\0'`` is a common notation for a null character, so rule ignores it.
