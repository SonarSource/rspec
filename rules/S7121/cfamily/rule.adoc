...
or `data()` on standard strings get introduced when the original string could be used directly.
...
and force to cross the content of a string to search for the null-terminator, reducing the performance.
