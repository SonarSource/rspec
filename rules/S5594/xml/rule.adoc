Once an Android component has been exported, both it and non-exported
components are exposed for untrusted use (more information about collateral damages on non-exported components
https://hackerone.com/reports/951691[here].)
...
**Warning**: The presence of intent filters will cause ``++exported++`` to be set to
``++true++`` by default.
