...
class GenericRule:
  ...
  def specializations(self) -> Generator[LanguageSpecificRule, None, None]:
    languages = utils.load_valid_languages()
    return (LanguageSpecificRule(child, self) for child in self.rule_path.iterdir() if child.is_dir() and child.name in languages)
