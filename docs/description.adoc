...
Content of the section "_3. How to fix it_ / _How to fix it in {Framework Display Name}_" can either be generic or framework specific.
...
When the content is framework-specific, one or more "_3. How to fix it in `{Framework Display Name}`_" sections (with their respective subsections) must be present.
...
This is important, as this format is
https://github.com/SonarSource/rspec/blob/21c463b6ed89cc570ed34f6883f994004f87ca5f/rspec-tools/rspec_tools/validation/description.py#L38[used by the CI to check the validity of the title],
and the same format will be expected by the analyzers when loading the rule content to recognize the different subsections.
...
A single and unique diff-id should be used only once for each type of code example in the description of a rule.
....
==== Noncompliant code example
[source,js,diff-id=1,diff-type=noncompliant]
----
